  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-10">
        <Section title="Accepted" tickets={accepted} onCardClick={handleCardClick} color="text-green-700" />
        <hr className="border-t-2 border-gray-300" />
        <Section title="In Process" tickets={inProcess} onCardClick={handleCardClick} color="text-yellow-700" />
        <hr className="border-t-2 border-gray-300" />
        <Section title="Completed" tickets={completed} onCardClick={handleCardClick} color="text-blue-700" />
      </div>
      ...
      {selectedTicket && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-[95%] max-w-5xl shadow-lg relative grid grid-cols-1 md:grid-cols-2 gap-8">
            <button onClick={closeModal} className="absolute top-2 right-3 text-gray-500 hover:text-red-600 text-xl font-bold">
              ×
            </button>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-gray-800">Logs & Comments</h3>

              <div className="bg-gray-100 p-3 rounded-lg h-48 overflow-y-auto">
                <ul className="text-sm text-gray-700 space-y-2">
                  {selectedTicket.logs?.map((log, index) => (
                    <li key={index} className="bg-white p-2 rounded shadow">{log}</li>
                  ))}
                </ul>
              </div>
                ...
                {attachments.map((file, index) => (
                  <div key={index}>
                    ...
                    {file.type.startsWith("image/") && (
                      <img src={file.url} alt={file.name} className="w-full max-h-40 object-contain rounded border" />
                    )}
                    ...
                    {file.type === "application/pdf" && (
                      <embed src={file.url} type="application/pdf" className="w-full h-48 border rounded" />
                    )}
                  </div>
                ))}
              </div>
...
function Section({ title, tickets, onCardClick, color }) {
  return (
    <section>
      <h3 className={`text-lg sm:text-xl font-semibold ${color} mb-4`}>{title}</h3>
